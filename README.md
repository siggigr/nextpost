...
- **A server-enforced progressive-disclosure security model.** A player can only ever read the post they're currently hunting for and the clues they've paid points to open — enforced by Firestore security rules ([`firestore.rules`](./firestore.rules), commented with the reasoning behind each widening), not by the UI politely hiding things. The negative cases were proven directly against the deployed rules in the Firebase Rules Playground — a signed-in user who has not joined is denied the game, its posts and its clues — and again in a live pass with two independent accounts on real devices.
